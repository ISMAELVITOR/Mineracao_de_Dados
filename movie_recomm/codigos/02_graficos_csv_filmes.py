...
def argumentos() -> argparse.Namespace:
    ...
    parser.add_argument("--saida", type=Path, default=Path("saida_movie_recomm/eda"))
...
def main() -> int:
    ...
    plt.boxplot(dados["media_avaliacoes"].dropna(), vert=True)
    plt.xticks([])
    ...
    plt.figure(figsize=(7, 5))
    plt.boxplot(dados["quantidade_avaliacoes"].dropna(), vert=True)
    plt.xticks([])
    ...
    for coluna, ordem, rotulos, titulo, arquivo in configuracoes_faixas:
        ...
        plt.bar(tabela["Rotulo"], tabela["Quantidade"])
        plt.title(titulo)
        plt.xlabel("Faixa")
        plt.ylabel("Quantidade de filmes")
        plt.xticks(rotation=10, ha="right")
        plt.grid(axis="y", alpha=0.25)
        salvar_figura(args.saida / arquivo)
